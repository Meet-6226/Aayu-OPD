...
const QUESTIONS = [
  {
    ...
    aiPurpose: 'Allows Aether AI to check for drug-disease contraindications and prepare your doctor with a pre-consultation summary.',
    options: [
  ...
  },
  {
    ...
    hint: 'Upload PDF, images, or docx records to train and calibrate your personal AI Care Assistant.',
    aiPurpose: 'Our OCR engine extracts key biomarkers and past vitals to update your clinical trendlines for the doctor.',
  },
];
...
function OptionChip({ label, selected, onClick }) {
  return (
    <button
      ...
      onClick={onClick}
      className={`
        px-4 py-3 text-xs font-semibold border text-left transition-all duration-200 rounded-[12px] w-full sm:w-[48.5%] flex items-center justify-between
        ${selected
          ? 'bg-[#e5f9f8] text-[#1b504c] border-[#97c9c4] shadow-[0_2px_8px_rgba(27,80,76,0.06)] font-bold'
          : 'bg-white text-[#475569] border-[#e2e8f0] hover:border-[#1E7F6A] hover:bg-[#FAFBFB] shadow-2xs'
        }
      `}
    >
      <span>{label}</span>
      <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-all ${
        selected ? 'border-[#1b504c] bg-[#1b504c] text-white' : 'border-[#cbd5e1] bg-transparent'
      }`}>
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-[#0f172a]/50 backdrop-blur-[3px] text-[#334155]">
      <motion.div
        ...
        className="bg-white w-full max-w-[960px] shadow-[0_10px_40px_rgba(0,0,0,0.08)] rounded-[20px] overflow-hidden border border-[#e2e8f0]"
      >
        <div className="grid grid-cols-1 md:grid-cols-12 gap-0">
          
          {/* ── LEFT PANEL ────────────────────── */}
          <div className="md:col-span-4 bg-gradient-to-b from-[#1b504c] to-[#112f2d] text-white p-6 flex flex-col justify-between text-left border-r border-white/5 select-none">

            {/* Branding — compact */}
            <div className="flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-[#86efac] shrink-0" />
              <span className="text-[10px] font-bold tracking-widest uppercase text-[#86efac]">Aether AI · ABDM</span>
            </div>
              ...
              {QUESTIONS.map((_, i) => (
                <div
                  ...
                  className={`h-1 rounded-full transition-all duration-300 ${
                    i < step + 1 ? 'bg-[#86efac]' : 'bg-white/15'
                  } ${i === step ? 'w-5' : 'w-2'}`}
                />
              ))}
            </div>
            ...
            <div className="flex-1 mt-6 bg-white/5 border border-white/10 rounded-[10px] p-4 flex flex-col gap-0.5 overflow-hidden">
              <div className="flex items-center gap-1.5 mb-3 pb-2 border-b border-white/10">
                <FileText className="h-3 w-3 text-[#86efac]" />
                <span className="text-[9px] font-bold text-white/70 uppercase tracking-widest">Clinical Draft</span>
              </div>
              ...
              {[
                ...
                <div key={label} className="grid grid-cols-[72px_1fr] gap-1 text-[9.5px] leading-tight py-1 border-b border-white/5 last:border-0">
                  ...
                  <span className="text-[#bbf7d0] font-mono truncate">{value}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-center gap-1.5 mt-4 text-[9px] text-white/30">
              <Shield className="h-3 w-3 shrink-0 text-[#86efac]/60" />
          ...
          <div className="md:col-span-8 flex flex-col justify-between">
            ...
            <div className="px-8 pt-7 pb-4 text-left">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-bold text-[#1E7F6A] tracking-wider uppercase bg-[#e5f9f8] px-2 py-0.5 rounded">
                  Clinical Check
                </span>
                ...
                >
                  {(q.type === 'multi-select' || q.type === 'single-select') && (
                    <div className="space-y-4">
                      ...
                      {isOtherSelected() && (
                        ...
                        >
                          <label className="block text-[10px] font-bold text-[#1b504c] uppercase tracking-wider mb-1">
                          ...
                          <input
                            ...
                            className="w-full px-3.5 py-2 text-xs border border-[#cbd5e1] focus:outline-none focus:border-[#1E7F6A] focus:bg-[#FAFBFB] rounded-[8px] text-[#111827] placeholder-[#94a3b8] font-sans"
                            autoFocus
                          />
                        </motion.div>
                      )}
                  ...
                  {q.type === 'textarea' && (
                    <div className="space-y-3">
                      <textarea
                        ...
                        rows={4}
                        className="w-full mt-2 px-4 py-3 rounded-[12px] border border-[#cbd5e1] text-xs text-[#111827] placeholder-[#94a3b8] focus:outline-none focus:border-[#1E7F6A] focus:bg-[#FAFBFB] resize-none transition-all"
                      />
                      ...
                      {q.id === 'current_medications' && (
                        <div>
                          <p className="text-[10px] font-bold text-[#64748b] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                            <Camera className="w-3 h-3 text-[#1E7F6A]" />
                            Medicine Photos
                            {medicinePhotos.length > 0 && (
                              <span className="ml-1 bg-[#1E7F6A] text-white text-[8.5px] font-bold px-1.5 py-0.5 rounded-full">
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            ...
                            {medicinePhotos.map((photo, idx) => (
                              <motion.div
                                ...
                                transition={{ duration: 0.15 }}
                                className="relative rounded-[10px] overflow-hidden border border-[#97c9c4] aspect-square group"
                              >
                                <img
                                  src={photo.preview}
                                  alt={`Medicine ${idx + 1}`}
                            ...
                            {medicinePhotos.length < MAX_MEDICINE_PHOTOS && (
                              <label className="aspect-square rounded-[10px] border-2 border-dashed border-[#cbd5e1] hover:border-[#1E7F6A] hover:bg-[#e5f9f8]/20 bg-[#FAFBFB] flex flex-col items-center justify-center cursor-pointer transition-all group">
                                <div className="w-8 h-8 bg-[#e5f9f8] group-hover:bg-[#c7f0eb] rounded-full flex items-center justify-center mb-1 transition-colors">
                                  <ImagePlus className="w-4 h-4 text-[#1E7F6A]" />
                                </div>
                                <span className="text-[9px] font-bold text-[#64748b] group-hover:text-[#1E7F6A] transition-colors text-center px-1 leading-tight">
                  ...
                  {q.type === 'file-upload' && (
                    <div className="pt-2 space-y-3">
                      {!uploadedFile ? (
                        <label className="flex flex-col items-center justify-center w-full h-32 rounded-[14px] border border-dashed border-[#cbd5e1] hover:border-[#1E7F6A] bg-[#FAFBFB] hover:bg-[#e5f9f8]/20 transition-all cursor-pointer group">
                          <div className="flex flex-col items-center justify-center p-4 text-center">
                            <UploadCloud className="w-7 h-7 text-[#1E7F6A] mb-2 transition-transform group-hover:scale-105" />
                        ...
                        <div className="p-3 border border-[#97c9c4] bg-[#e5f9f8]/30 flex items-center justify-between rounded-[10px]">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-[#1E7F6A]/10 text-[#1E7F6A] flex items-center justify-center rounded-[8px] shrink-0">
                              <FileText className="w-4 h-4" />
                            </div>
                            <div className="text-left overflow-hidden">
                              <p className="text-xs font-bold text-[#111827] truncate max-w-[280px]">
                                {uploadedFile.name}
                              </p>
                          ...
                          <button
                            onClick={removeFile}
                            className="p-1.5 hover:bg-[#e5f9f8] text-red-500 transition-colors rounded-[8px]"
                          >
                      ...
                      <div className="p-3 border border-[#e2e8f0] bg-[#FAFBFB] flex items-start gap-2.5 rounded-[10px]">
                        <Sparkles className="w-4.5 h-4.5 text-[#1E7F6A] shrink-0 mt-0.5 animate-pulse" />
            ...
            {/* AI Purpose Box */}
            <div className="mx-8 mb-2 p-3 bg-[#FAFBFB] border border-[#e2e8f0] rounded-[10px] flex items-start gap-2.5 text-left transition-all duration-150">
              <Info className="h-4 w-4 text-[#1E7F6A] shrink-0 mt-0.5" />
              <div className="space-y-0.5">
                <span className="text-[9px] font-bold text-[#1E7F6A] uppercase tracking-wider block">Why Aether AI asks this:</span>
                <p className="text-[10.5px] text-[#475569] leading-normal font-medium">{q.aiPurpose}</p>
              </div>
            </div>
            ...
            <div className="px-8 py-5 flex items-center justify-between border-t border-[#e2e8f0]">
              <button
                ...
                disabled={step === 0}
                className="flex items-center gap-1 text-xs font-bold text-[#64748b] hover:text-[#111827] disabled:opacity-30 disabled:cursor-not-allowed transition-all"
              >
              ...
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="text-xs font-bold text-[#64748b] hover:text-[#111827] underline underline-offset-2 transition-colors"
                >
                  Skip
                </button>
                ...
                <button
                  ...
                  className={`
                    flex items-center gap-1.5 px-5 py-2.5 text-xs font-bold transition-all duration-200 rounded-[10px] shadow-sm
                    ${canProceed() && !submitting
                      ? 'bg-[#1E7F6A] hover:bg-[#165B52] text-white cursor-pointer shadow-[#1E7F6A]/10'
